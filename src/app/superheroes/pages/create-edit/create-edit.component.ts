...
export class CreateEditComponent implements OnInit {
	...
	protected modifySuperheroe(name: string, fullName: string, publisher: string) {
		const heroeSelectedId = this.heroeSelectedId();
	...
	protected createSuperheroe(name: string, fullName: string, publisher: string) {
	...
	protected openSnackBar() {
		this.snackBar.openFromComponent(SnackbarComponent, {
			...
			data: {
				id: '1',
				message: `Superheroe ${this.form.controls.name.value} ${this.editSuperheroe() ? 'editado' : 'creado'}`
			}
		});
	}
}
